...
public class TweenPosition : UITweener
{
    ...
    void Start()
    {
        if (this.name.Equals("OtherFunctionSet"))
        {
            print("this.name = " + this.name);
            GameObject dingWei = GameObject.Find("Sprite_dingweidian");
            Vector3 tmpFrom = dingWei.transform.localPosition;
            Vector3 tmpTo = tmpFrom;
            print("tmpForm = " + tmpFrom);
            print("tmpTO = " + tmpTo);
            tmpFrom.x -= 309.0f;
            tmpFrom.y -= 305.5f;
            from = tmpFrom;
