/*此函数用于显示小地图，showpoints函数用于在小地图上显示红色标志，CheckForClicksOnMinimap()用于点击红色标志生成路线。当点击不同的地点时，需要调用OnTargetReached()函数
 * 但是此函数不能在调用aipath时调用，所以在BotAI中调用。
 * 
 * */
...
public class KGFMapSystem : KGFModule, KGFICustomGUI, KGFIValidator, KGFICustomInspectorGUI
{
    ...
    public static bool IsChangeTarget = false;
    public static Vector3 Near;
    public Texture2D ittexture;         //一楼的地图
    public Texture2D ittexture1;        //一楼的地图
    public Texture2D ittexture2;        //二楼的地图
    public Texture2D ittexture3;        //三楼的地图
    Vector3 po;
    ...
    public static bool To_Target;    //      判断是否到达了target
    public static Vector3[] Target_Flag = new Vector3[20];       //目标的位置
    private int Target_num;         //target的下标

    public KGFMapSystem()
        : base(new Version(1, 1, 0, 0), new Version(1, 1, 0, 0))
    { }
    ...
    /// </summary>
    public static Transform itsContainerFlags;
    /// <summary>
    ...
    {
        //DeletePoints();
        //print("Seeker.lastCompletedVectorPath.Count = " + Seeker.lastCompletedVectorPath.Count);
       ...
       // Vector3 popo;
        ...
        if (Seeker.lastCompletedVectorPath != null && Seeker.Exist_target)
        {
            lineRenderer.SetVertexCount(Seeker.lastCompletedVectorPath.Count);
            for (int i = 0; i < Seeker.lastCompletedVectorPath.Count; i++)
            {
                ...
                float x = Seeker.lastCompletedVectorPath[i].x;
                float y = Seeker.lastCompletedVectorPath[i].y;
                float z = Seeker.lastCompletedVectorPath[i].z;
                /*ChangePosition changepositiont = new ChangePosition();
                changepositiont.ChangePoint(x, y, z);
                popo.x = changepositiont.GetX1();
                popo.y = changepositiont.GetY1();
                popo.z = changepositiont.GetZ1();*/
                Vector3 aClickWorldPoint = new Vector3(x, y, z);
                aClickWorldPoint.y = itsFixedYFlags + 3.0f;
                lineRenderer.SetPosition(i, aClickWorldPoint);
                /* Vector3 aClickWorldPoint = new Vector3(x, y, z);
                 aClickWorldPoint.y = itsFixedYFlags;
                 //print("x = " + aClickWorldPoint.x);
                // print("y = " + aClickWorldPoint.y);
                // print("z = " + aClickWorldPoint.z);
                // print("x = " + Seeker.lastCompletedVectorPath[i].x);
                // print("y = " + Seeker.lastCompletedVectorPath[i].y);
                // print("z = " + Seeker.lastCompletedVectorPath[i].z);
                // print("******");
    ...
    /*public void ShowTarget()
    {
        ...
        //DeletePoints();             //首先删除之前生成的红色标志，之后再生成新的红色标志，避免冲突
        // check for click
        // CreateCameras();
        Vector3 po;
        float x, y, z;
        //Thread.Sleep(3000);
        //print("It has Sleeping!!!");
        if (AddTarget.Target_Count == 0)
            return;
        po = Target_Flag[Target_num];
        print("po = " + po);
        ...
        //x = ;
        Vector3 aClickWorldPoint = new Vector3(po.x, po.y, po.z);
        aClickWorldPoint.y = itsFixedYFlags;
       // print("******");
        // check if there is already a user flag at this position
        bool aFoundUserFlag = false;
        ...
        /*for (int i = 0; i < ControlChange.cnt; i++)
        {
            ...
            // print("po.y = " + po.y);
            // print("po.z = " + po.z);
            //Vector2 aMousePosition = new Vector2(po.x, po.y);
            ...
            /* Vector2 aPercentofImageClick = new Vector2((aMousePosition.x - itsTargetRect.x) / (itsTargetRect.width),
             ...
             Vector3 aClickWorldPoint = new Vector3();
             aClickWorldPoint = itsCameraTransform.position +
    ...
    protected override void KGFAwake()
    {
        ...
        foreach (KGFIMapIcon anIcon in KGFAccessor.GetObjects<KGFIMapIcon>())
            RegisterIcon(anIcon);
        target1 = itsDataModuleMinimap.itsTarget;
    ...
    void AutoCreatePhoto()
    {
        ...
        if (itsDataModuleMinimap.itsPhoto.itsTakePhoto)
        {
            ...
            aTempCameraGameObject.transform.position = itsTerrainBounds.center + new Vector3(0, itsTerrainBounds.extents.y - 5, 0);
            ...
            // render to texture
            Texture2D aPlaneTexture = ittexture;            //修改小地图的图片
            {
            ...
            // show photo in plane under terrain
            {
                ...
                Vector3 aPhotoPlanePosition = itsTerrainBounds.center;

                aPhotoPlanePosition.x -= 125.45022f;
                aPhotoPlanePosition.y = itsFixedYPhoto;
                aPhotoPlanePosition.z -= 19.79729f;
                aPhotoPlane.transform.position = aPhotoPlanePosition;
                aPhotoPlane.transform.localScale = new Vector3(itsTerrainBounds.size.x - 170.5113f, 1, itsTerrainBounds.size.z - 179.11401f);
                /*
                aPhotoPlanePosition.z -= 45;
                aPhotoPlanePosition.x -= 113;
                aPhotoPlane.transform.position = aPhotoPlanePosition;
                aPhotoPlane.transform.localScale = new Vector3(itsTerrainBounds.size.x - 70, 1, itsTerrainBounds.size.z - 165);*///Vector3.one * Mathf.Max(aBounds.size.x,aBounds.size.z);
                //修改小地图的大小
            }
        }
    }
    ...
    void CreateCameras()
    {
        ...
        print("itsCamera.transform.position.yc = " + itsCamera.transform.position.y);
        itsCameraTransform = itsCamera.transform;
        itsCameraTransform.position = new Vector3(0.0f, 100.0f, 0.0f);
    ...
    void UpdateMinimapOutputPlane()
    {
        ...
        //print("itsCamera.transform.position.yc = " + aCameraThatFilmsPlane.transform.position.y);
    ...
    void Update()
    {
        ...
        Profiler.BeginSample("ShowLines()");
        ShowLines();
        Profiler.EndSample();

       /* Profiler.BeginSample("ShowTarget()");
        ShowTarget();
        Profiler.EndSample();*/
    ...
    //切换楼层的时候会切换小地图，并且函数中可以修改小地图的大小参数
    void UpdateMapIconRotation()
    {
        // print("itsCamera.transform.position.yc = " + itsCamera.transform.position.y);
        po = target1.transform.position;
        if (po.y > 20.0f)
        {
            if (ittexture != ittexture3)
            {
                ittexture = ittexture3;
                flag = true;
                flag3 = true;
            }
        }
        else if (po.y > 10.0f && po.y < 20.0f)
        {
            if (ittexture != ittexture2)
            {
                ittexture = ittexture2;
                flag = true;
                flag2 = true;
            }
        }
        ...
        {
            if (ittexture != ittexture1)
            {
                ittexture = ittexture1;
                flag = true;
                flag1 = true;
            }
        }
        ...
        {               //如果满足条件，则更换掉小地图的图片
            flag = false;
            DeletePoints();     //切换楼层时，用于清空生成的红色标志
            GameObject temp = null;
            temp = GameObject.Find("photo_plane"); //清楚之前生成的GameObject，节约空间
            ...
            if (itsDataModuleMinimap.itsPhoto.itsTakePhoto)
            {
                ...
                // show photo in plane under terrain
                {
                    ...
                    Vector3 aPhotoPlanePosition = itsTerrainBounds.center;
                    aPhotoPlanePosition.y = itsFixedYPhoto;
                    //设置flag的目的是因为楼层之间的位置及大小不一，因此需要分开处理
                    if (flag1)
                    {
                        flag1 = false;
                        aPhotoPlanePosition.x -= 125.45022f;
                        ...
                        aPhotoPlane.transform.position = aPhotoPlanePosition;
                        aPhotoPlane.transform.localScale = new Vector3(itsTerrainBounds.size.x - 170.5113f, 1, itsTerrainBounds.size.z - 179.11401f);
                        //Vector3.one * Mathf.Max(aBounds.size.x,aBounds.size.z);
                    }
                    else if (flag2)
                    {
                        flag2 = false;
                        aPhotoPlanePosition.x -= 127.97507f;
                        aPhotoPlanePosition.z -= 72.03638f;
                        aPhotoPlane.transform.position = aPhotoPlanePosition;
                        aPhotoPlane.transform.localScale = new Vector3(itsTerrainBounds.size.x - 159.3349f, 1, itsTerrainBounds.size.z - 142.973f);//Vector3.one * Mathf.Max(aBounds.size.x,aBounds.size.z);
                    }
                    else if (flag3)
                    {
                        flag3 = false;
                        aPhotoPlanePosition.x -= 128.58523f;
                        aPhotoPlanePosition.z -= 72.93726f;
                        aPhotoPlane.transform.position = aPhotoPlanePosition;
                        aPhotoPlane.transform.localScale = new Vector3(itsTerrainBounds.size.x - 158.9855f, 1, itsTerrainBounds.size.z - 141.6107f);
                    }
                    //修改小地图的大小
                }
            }
    ...
    void UpdateOrthographicSize()      //触屏缩放小地图的大小
    {
        if (Input.touchCount > 1)
        {
            ...
            if (Input.GetTouch(0).phase == TouchPhase.Moved || Input.GetTouch(1).phase == TouchPhase.Moved)
            {
                ...
                if (isEnlarge(oldPosition1, oldPosition2, tempPosition1, tempPosition2))
                {
                    ...
                    itsPixelPerMeter = GetZoom() + itsDataModuleMinimap.itsZoom.itsZoomChangeValue;
                }
                else
                {
                    itsPixelPerMeter = GetZoom() - itsDataModuleMinimap.itsZoom.itsZoomChangeValue;
        ...
        //UILabel myLabel = GameObject.Find("Label_amp").GetComponent<UILabel>();
        // myLabel.text = itsCamera.orthographicSize.ToString();
        UpdateIconScale();
